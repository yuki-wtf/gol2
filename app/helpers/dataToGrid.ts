...
function toBinaryStringArray(val) {
  return (
    ...
    val // Turn integer into a binary string
      .toString(2) // Turn it into an array
      .split('') // Convert every value to a number
      .map((v) => parseInt(v, 10))
  )
}
...
export function dataToGrid(data): number[][] {
  if ((data && data !== null) || data !== undefined) {
    return Object.keys(data).map((key) => {
      ...
      return emptyRow // Remove the last part of the empty row to make space for the "end" array
        .slice(0, GRID_SIZE - end.length) // Then append the "end" array to the end
        .concat(end)
    })
  }
}
