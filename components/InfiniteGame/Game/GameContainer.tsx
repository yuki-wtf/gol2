...
const GameContainer = () => {
  ...
  const { latest_generation, generations } = useSelector((state) => state.generations)
  useEffect(() => {
    setTimeout(() => {
      setLUS(false)
    }, 10000)
  ...
  return (
    <StyledGridContainer>
      ...
      {latest_generation && missingGensIncrements.length > 0 && callIncrement > 0 ? (
        <GetPreviousGamesLarge missingGens={missingGensIncrements[callIncrement]} />
      ) : null}
...
export default GameContainer // get latest game state
// const {
